...
// CODE OBTAINED FROM - https://codepen.io/FRADAR/pen/BaRoPJW

const BugDispatch = {
  transform: null as ((s: string) => void) | null,
  transforms: {} as { [key: string]: any },
  modes: [] as string[],
  bugs: [] as any[],
  spawnDelay: [] as any[],
  multiplyDelay: false,
  options: {
    ...
    bugHeight: 14,
    num_frames: 5,
    zoom: 10, // random zoom letiation from 1 to 10 - 10 being full size.
    canFly: true,
    canDie: true,
    numDeathTypes: 3,
  ...
  add_events_to_bug: function (thebug: any) {
    const that = this;
    if (thebug.bug) {
  ...
  check_if_mouse_close_to_bug: function (e?: any) {
    e = e || window.event;
    if (!e) {
      return;
    }
  ...
  near_bug: function (bug: any) {
    this.on_bug(bug);
  },

  on_bug: function (bug: any) {
    if (!bug.alive) {
      return;
    }
  ...
  random: function (min: number, max: number, round?: boolean) {
...
export const FlyController = function (this: any, ...args: any[]) {
  this.initialize(...args);
};
FlyController.prototype = BugDispatch;

export const SpiderController = function (this: any, ...args: any[]) {
  const spiderOptions = {
...
const Bug: any = {
  ...
  initialize: function (transform: any, options: any) {
    this.options = mergeOptions(this.options, options);
  ...
  go: function () {
    if (this.transform) {
      ...
      this.going = requestAnimationFrame(function (t) {
        that.animate(t);
      });
    }
  },
  ...
  setPos: function (top: number, left: number) {
    this.bug.top =
      top ||
  ...
  moveBug: function (x: string, y: string, deg: number) {
  ...
  drawBug: function (top: number, left: number) {
    if (!this.bug) {
      this.makeBug();
    }
    if (!this.bug) return;
  ...
  fly: function (landingPosition: { top: number; left: number }) {
  ...
  flyRand: function () {
    this.stop();
    const landingPosition: { top: number; left: number } = { top: 0, left: 0 };
  ...
  startFlying: function (landingPosition: { left: number; top: number }) {
    const currentTop = this.bug.top,
      currentLeft = this.bug.left,
  ...
  drop: function (deathType: any) {
    const startPos = this.bug.top,
      d = document,
      e = d.documentElement,
      g = d.getElementsByTagName('body')[0],
      finalPos = (window.innerHeight || e.clientHeight || g.clientHeight) - this.options.bugHeight,
      rotationRate = this.random(0, 20, true),
      that = this;

    this.bug.classList.add('bug-dead');

    this.dropTimer = requestAnimationFrame(function (t: any) {
      that._lastTimestamp = t;
      that.dropping(t, startPos, finalPos, rotationRate, deathType);
    });
  },

  dropping: function (t: number, startPos: number, finalPos: number, rotationRate: any, deathType: number) {
    ...
    if (newPos >= finalPos) {
      ...
      const rotationOffset = (this.options.bugWidth * this.zoom - this.options.bugHeight * this.zoom) / 2;
  ...
  twitch: function (deathType: number, legPos: number) {
    //this.bug.style.back
    if (!legPos) legPos = 0;
    const that = this;
  ...
  random: function (min: number, max: number, plusminus?: boolean) {
...
const SpawnBug = function () {
  const newBug = Object.create(Bug);
  return newBug;
};
...
const mergeOptions = function (obj1: any, obj2: any, clone?: any) {
  if (typeof clone == 'undefined') {
    clone = true;
  }
  const newobj = clone ? cloneOf(obj1) : obj1;
...
const cloneOf = function (
  obj: { [x: string]: any; constructor: () => any; hasOwnProperty: (arg0: string) => any } | null,
) {
